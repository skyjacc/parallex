...
export async function PATCH(req: Request) {
    ...
    try {
        const { id, role, prxBalance, flagged, discordId, discordTag } = await req.json();
        ...
        if (role === "ADMIN" || role === "USER" || role === "SUPPORT") data.role = role;
        if (typeof prxBalance === "number" && prxBalance >= 0) data.prxBalance = prxBalance;
        if (typeof flagged === "boolean") data.flagged = flagged;
        if (typeof discordId === "string") data.discordId = discordId || null;
        if (typeof discordTag === "string") data.discordTag = discordTag || null;
